...
def Length(bar):
    ...
    v1 = BarNodeToVector(bar_node, bar)
...
def BarsToVectors(bar_1,bar_2):
    origin_node = FindSharedNode(bar_1, bar_2)
    v1 = BarNodeToVector(origin_node, bar_1)
    v2 = BarNodeToVector(origin_node, bar_2)
    return v1,v2

# Cross product of two vectors
def TwoDCrossProduct(vec1,vec2):
    v1xv2 = vec1[0]*vec2[1]-vec1[1]*vec2[0]
    return v1xv2
...
def CosineVectors(local_x_vec,other_vec):
    cosine = (DotProduct(local_x_vec, other_vec))/(VectorTwoNorm(local_x_vec)*VectorTwoNorm(other_vec))
    return cosine

# Sine of angle from local x vector direction to other vector
def SineVectors(local_x_vec,other_vec):
    sine = (TwoDCrossProduct(local_x_vec, other_vec))/(VectorTwoNorm(local_x_vec)*VectorTwoNorm(other_vec))
    return sine
